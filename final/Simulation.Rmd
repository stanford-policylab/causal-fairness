...
Prints out the results of a given admissions method. A "book of life" is a draw of each potential outcome for a population of a given
...
HUMAN_RESULTS = top_k(df_test, df_test$A, POP_SIZE_TEST_ADMIT, 'Human Decision')
HUMAN_RESULTS_NON_RANDOM = top_k(df_test, df_test$A_prob, POP_SIZE_TEST_ADMIT, 'Human Decision')

df_train%>% select(R, A) %>%
 group_by(R) %>%
 summarize(admit_rate = mean(A))

top_k <- function(DF_TEST, DF_TEST_COL, N_ADMIT, RESULT_NAME){
...
ML_NAIVE_RESULTS = top_k(df_test, df_test$ml_naive, POP_SIZE_TEST_ADMIT, 'ML Naive')
...
ML_BOOST_2 = top_k(df_test, df_test$ml_outcomes, POP_SIZE_TEST_ADMIT, 'BLACK + 0.36')
...
ML_BOOST_2 = top_k(df_test, df_test$ml_outcomes, POP_SIZE_TEST_ADMIT, 'BLACK + 0.35')



ML_MAX_BOARDS_PASSAGE = top_k(df_test, df_test$ml_outcomes, POP_SIZE_TEST_ADMIT, 'Max Graduation')
#expression(atop("Histogram of "*hat(mu), Bootstrap~samples*','~Allianz))


ML_MAX_UTILITY = top_k(df_test, df_test$ml_outcomes_div, POP_SIZE_TEST_ADMIT,  expression("Maximum utility (when " * lambda ~ " = 0.01)"))
...
# FRAC_ADMIT
# 
# ML_PREDICT_PAR= top_k(df_test, df_test$ml_outcomes_pred_par, POP_SIZE_TEST_ADMIT, 'Counterfactual Pred. Parity')
# 
# 
# 
# df_pred_par <-  df_test%>% select(R, B_p, ml_outcomes_pred_par_decision) %>%
...
ML_PREDICT_PAR = c('Counterfactual Pred. Parity', 0, lp_results[lp_results$Policy.Name=='Counterfactual Pred. Parity',]$Frac.Diverse.Candidates, lp_results[lp_results$Policy.Name=='Counterfactual Pred. Parity',]$Total.Graduated/ POP_SIZE_TEST_ADMIT)
...
p <- ggplot() +
  ...
  coord_cartesian(xlim = c(0.0, 0.60),ylim = c(0.437, 0.635),expand = FALSE) + 
  ...
  annotate("text",size=5, x = as.double(ML_MAX_UTILITY[[3]])-0.06+0.075-0.01, y = as.double(ML_MAX_UTILITY[[4]])-500/ POP_SIZE_TEST_ADMIT, label = expression("Max Utility (for " * lambda ~  " = " * frac(1, 4) *")")) +
