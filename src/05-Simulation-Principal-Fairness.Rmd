...
```{r trace_results}
  ...
  print_results <- function(DF_TEST, DF_TEST_COL, N_ADMIT, RESULT_NAME){
    ...
    cat("Boards Passage Count: ", sum(head(DF_TEST,n=N_ADMIT)$B_p),'\n')
...
gen_book_of_life <- function(
  ...
  book_of_life <- tibble(
      ...
      R_string = if_else(R==1,'Black','White'),
      ...
      B_p = ifelse(M > quantile(M,0.75), 1,0),
...
## Conditional on T, E, is race independent of med school preparation
```{r ml_naive}
head(df_train)[,c('R','E','M','T')]

hist(df_train$L_p)

ggplot(df_train, aes(x=L_p, fill=R_string)) +
  geom_histogram()
...
ML_NAIVE_RESULTS = return_results(df_test, df_test$ml_naive, POP_SIZE_TEST_ADMIT, 'ML Max Lives Saved')
...
AA_DECISION = mean(FRAC_BLACK*predict(ml_naive,aa_input_black) + (1-FRAC_BLACK)*predict(ml_naive,aa_input_white))
...
p <- ggplot() +
  ...
  annotate("text", x = as.double(ML_AA_FAIR_RESULTS[[3]])-0.05, y = as.double(ML_AA_FAIR_RESULTS[[2]]) + 1000, label = ML_AA_FAIR_RESULTS[[1]]) +
  ...
  annotate("text", x = as.double(ML_EO_FAIR_RESULTS[[3]])-0.03, y = as.double(ML_EO_FAIR_RESULTS[[2]]) + 1000, label = ML_EO_FAIR_RESULTS[[1]]) +
  
  geom_vline(xintercept = as.double(ML_NAIVE_RESULTS[[3]]), linetype = "dashed", colour = green, alpha=transparency) +
  geom_hline(yintercept = as.double(ML_NAIVE_RESULTS[[2]]), linetype = "dashed", colour = green, alpha=transparency) +
  geom_point(aes(x = as.double(ML_NAIVE_RESULTS[[3]]), y = as.double(ML_NAIVE_RESULTS[[2]])), color = green, size = 3) +
  annotate("text", x = as.double(ML_NAIVE_RESULTS[[3]])+0.03, y = as.double(ML_NAIVE_RESULTS[[2]]) + 1000, label = ML_NAIVE_RESULTS[[1]]) +
