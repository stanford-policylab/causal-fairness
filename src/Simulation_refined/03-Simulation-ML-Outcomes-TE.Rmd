...
```{r ml_naive}
df_train_ = filter(df_train, A == 1 )
ml_naive <- lm(L_p ~ T + R + E , data = df_train_)
...
ml_naive_pf <- lm(L_p ~ T + E, data = df_train_)
...
aa_probs_black <- df_train %>%
  count(R, T, E, T_black, E_black) %>%
  group_by(R, T, E) %>%
  mutate(p = n / sum(n)) %>%
  ungroup()
  
aa_probs_white <- df_train %>%
  count(R, T, E, T_white, E_white) %>%
  group_by(R, T, E) %>%
  mutate(p = n / sum(n)) %>%
  ungroup()


AA_FAIR_MODEL <- function(applicant_race, applicant_test, applicant_edu, I_noise){
  
# Get sampling space for black and white counterfactuals
aa_probs_black_sample <- aa_probs_black %>% 
filter(R == applicant_race, T==applicant_test, E==applicant_edu)
...
sampled_edu_scores = aa_probs_black_sample[sample.int(nrow(aa_probs_black_sample), COUNTER_FACTUAL_SAMPLE*FRAC_BLACK, replace = TRUE, prob = aa_probs_black_sample$p),]$E_black
sampled_edu_scores = c(sampled_edu_scores,aa_probs_white_sample[sample.int(nrow(aa_probs_white_sample), COUNTER_FACTUAL_SAMPLE*(1-FRAC_BLACK), replace = TRUE, prob = aa_probs_white_sample$p),]$E_white)


aa_input_black <- tibble(R=1, T=sampled_test_scores , E=sampled_edu_scores , I_noise = I_noise)
aa_input_white <- tibble(R=0, T=sampled_test_scores , E=sampled_edu_scores, I_noise = I_noise)
...
df_test <- df_test %>% 
  rowwise() %>% 
  mutate(ml_aa_fair = AA_FAIR_MODEL(R,T, E,I_noise))
...
p <- ggplot() +
 ...
 # annotate("text", x = as.double(ML_AA_FAIR_RESULTS[[3]])-0.05, y = as.double(ML_AA_FAIR_RESULTS[[4]]) + 2, label = ML_AA_FAIR_RESULTS[[1]]) +
  ...
  annotate("text", x = as.double(ML_PF_RESULTS[[3]])+0.03, y = as.double(ML_PF_RESULTS[[4]]) + 12, label = ML_PF_RESULTS[[1]]) +
