...
gen_book_of_life <- function(
  ...
  f_M <- function(e_i, noise, race){
    return(round(m_0 + m_e*e_i + noise - race*10,digits=0))
  }
  ...
  book_of_life <- tibble(
      ...
      M_black = f_M(E_black, noise=M_noise, race=1),
      M_white = f_M(E_white, noise=M_noise, race=0),
...
df_test = rowid_to_column(df_test, "applicant_id")
df_train = rowid_to_column(df_train, "applicant_id")
...
pfCheckT_E <- function(df_check, includeE=TRUE) {
  B_p = 'B_p'
  R = 'R'
  A = 'A'
  A_max = 'A_max'
  ...
  if (includeE) {
    df_check_ <- df_check[,c(A_max, R, B_p, T, E)]  %>% 
      group_by(R, T, E, B_p) %>%
...
df_test$A_max = ifelse(df_test$ml_outcomes_e > quantile(df_test$ml_outcomes_e,0.9),1,0)
df_test$A_max = df_test$A
cat(pfCheckT_E(df_test,includeE=TRUE))
...
for (num_black_admit in c(13629,15895,seq(from=1,to=as.integer(MAX_BLACK_ADMIT),by=2000))){
  cat(num_black_admit)
  cat('\n')
  ...
  admits <-  c(head(df_test_black,n=num_black_admit)$applicant_id,
  head(df_test_white,n=POP_SIZE_TEST_ADMIT-num_black_admit)$applicant_id)
  df_test$A_max = ifelse(match(df_test$applicant_id,admits) > 0, 1,0) %>% replace(is.na(.), 0)
...
p <- ggplot() +
 ...
 # annotate("text", x = as.double(ML_AA_FAIR_RESULTS[[3]])-0.05, y = as.double(ML_AA_FAIR_RESULTS[[4]]) + 2, label = ML_AA_FAIR_RESULTS[[1]]) +
  ...
  annotate("text", x = as.double(ML_NAIVE_RESULTS[[3]])+0.01, y = as.double(ML_NAIVE_RESULTS[[4]]) - 1000, label = ML_NAIVE_RESULTS[[1]]) +
