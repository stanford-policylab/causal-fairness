...
df_test$ml_outcomes_div = df_test$ml_outcomes + 0.275*df_test$R
...
EO_FAIR = c('Counterfactual Equalized Odds', 21017, 0.12221, 21017)
CF_FAIR = c('Counterfactual Fairness', 9635, 0.20, 9635)
RANDOM = c('Random Admissions', 6211, 0.20, 6211)
...
p <- ggplot() +
  ...
  annotate("text", x = as.double(ML_PREDICT_PAR[[3]])+0.05, y = as.double(ML_PREDICT_PAR[[4]]) - 50, label = ML_PREDICT_PAR[[1]]) +
  ...
  annotate("text", x = as.double(EO_FAIR[[3]]), y = as.double(EO_FAIR[[4]]) - 50, label = EO_FAIR[[1]]) +

  
        geom_vline(xintercept = as.double(CF_FAIR[[3]]), linetype = "dashed", colour = purple, alpha=transparency) +
  geom_hline(yintercept = as.double(CF_FAIR[[4]]), linetype = "dashed", colour = purple, alpha=transparency) +
  geom_point(aes(x = as.double(CF_FAIR[[3]]), y = as.double(CF_FAIR[[4]])), color = purple, size = 3) +
  annotate("text", x = as.double(CF_FAIR[[3]])-0.05, y = as.double(CF_FAIR[[4]]) - 50, label = CF_FAIR[[1]]) +
