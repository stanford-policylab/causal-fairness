...
grid_ <- grid %>%  group_by(min_black_admits) %>% summarize( 
...
p <- ggplot() +
  ...
  geom_ribbon(data=grid_,aes(x=min_black_admits,ymin = y_min, ymax = y_max), color="#C04BFF50",fill="#C04BFF50",linetype=0.5) +
...
p <- ggplot() +
  ...
  geom_line(data= filter(pareto_ideal, cutoff),aes(x=frac_minority, y=smooth_y, color='ML Outcomes (Given T, R)'),size=1.5) + 
  geom_line(data= filter(pareto_ideal, !cutoff),aes(x=frac_minority, y=smooth_y, color='ML Outcomes (Given T, R)'), linetype = "dotted",size=1.5) + 
  geom_ribbon(data=grid_,aes(x=min_black_admits,ymin = y_min, ymax = y_max), color=NA,fill="#BED05650",linetype=0,size=0.0) +
...
p <- ggplot() +
  ...
  geom_line(data= filter(pareto_ideal, cutoff),aes(x=frac_minority, y=smooth_y, color='ML Outcomes (Given T, R)'),size=1.5) + 
  geom_line(data= filter(pareto_ideal, !cutoff),aes(x=frac_minority, y=smooth_y, color='ML Outcomes (Given T, R)'), linetype = "dotted",size=1.5) + 
 geom_ribbon(data=grid_,aes(x=min_black_admits,ymin = y_min, ymax = y_max), color="NA",fill="#E9A35950",linetype=0.5) +
...
p <- ggplot() +
  ...
  geom_line(data= filter(pareto_ideal, cutoff),aes(x=frac_minority, y=smooth_y, color='ML Outcomes (Given T, R)'),size=1.5) + 
  geom_line(data= filter(pareto_ideal, !cutoff),aes(x=frac_minority, y=smooth_y, color='ML Outcomes (Given T, R)'), linetype = "dotted",size=1.5) + 
  geom_ribbon(data=grid_,aes(x=min_black_admits,ymin = y_min, ymax = y_max), color="NA",fill="#69FF4B50",linetype=0.5) +
  ...
  scale_color_manual(
      values = c('ML Outcomes (Given T, R)'="violet" )) + 
  theme(legend.title = element_blank(),plot.margin = margin(0.25,1,0.25,0.5, "cm"),legend.position="none", axis.text=element_text(size=18),plot.title = element_text(size=22, hjust=0.5)), axis.title=element_text(size=22), panel.grid = element_line(color = rgb(235, 235, 235, 100, maxColorValue = 255)))
