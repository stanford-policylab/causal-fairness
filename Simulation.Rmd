...
```{r trace_results}
  ...
  return_results <- function(DF_TEST, DF_TEST_COL, N_ADMIT, RESULT_NAME){
    DF_TEST<-DF_TEST[order(DF_TEST_COL,decreasing=TRUE),]
    return(c(RESULT_NAME, "Results", sum(head(DF_TEST,n=N_ADMIT)$D_p)/N_ADMIT, sum(head(DF_TEST,n=N_ADMIT)$B_p)))
  }
...
PC_FAIR = c('Counterfactual Fairness\n& Path-Specific Fairness\n(Random)', 0, lp_results[lp_results$Policy.Name=='Counterfactual Fairness',]$Frac.Diverse.Candidates, lp_results[lp_results$Policy.Name=='Counterfactual Fairness',]$Total.Graduated)
EO_FAIR = c('Counterfactual Equalized Odds', 0, lp_results[lp_results$Policy.Name=='Counterfactual Equalized Odds',]$Frac.Diverse.Candidates, lp_results[lp_results$Policy.Name=='Counterfactual Equalized Odds',]$Total.Graduated)
PRINCIPAL_FAIR = c('Principal Fairness', 0, lp_results[lp_results$Policy.Name=='Principal Fairness',]$Frac.Diverse.Candidates, lp_results[lp_results$Policy.Name=='Principal Fairness',]$Total.Graduated)
```
...
ggsave(plot=p, filename='./frontier.pdf', height=5, width=7)
p
