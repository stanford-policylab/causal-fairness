...
p2 <- p1 +
  stat_function(fun = ~ 1/2 * dbeta(., 2, 3), geom = "area") 
...
p2.5 <- p2 +
  geom_vline(xintercept = m2, linetype = "dashed", color = "red", size = 2)
...
m3 <- tibble(x = rbeta(1e7, 2, 3)) %>%
  filter(x > qbeta(1/2, 2, 3)) %>%
  with(mean(x))

p3 <- p1 +
  stat_function(fun = ~ dbeta(., 2, 3), geom = "area", xlim = c(qbeta(1/2, 2, 3), 1))
...
p6 <- p4 +
  stat_function(
    fun = ~ dbeta(., 3, 2),
    geom = "area",
    xlim = c(qbeta(2/5, 3, 2), 1),
    data = tibble(A = "Majority")
  ) +
  stat_function(
    fun = ~ dbeta(., 2, 3),
    geom = "area",
    xlim = c(qbeta(3/5, 2, 3), 1),
    data = tibble(A = "Minority")
  )
...
p8 <- ggplot() +
  ...
  geom_function(fun = ~ dbeta(., 2, 2), data = tibble(a = "Majority", y_ = "Would graduate\nif admitted")) +
...
p9 <- p8 +
  ...
  annotate("text", x = 2/3, y = 1/2, label = "50%", color = "white", size = 5)
...
p12 <- p10 +
  geom_area(stat = "function", fun = ~ dbeta(., 2, 2), xlim = c(t_maj, 1),
            data = tibble(a = "Majority", y_ = "Would graduate\nif admitted")) +
  geom_text(label = str_c(format(100 - 100 * pbeta(t_maj, 2, 2), digits = 2), "%"),
            data = tibble(a = "Majority", y_ = "Would graduate\nif admitted",
                          x = 2/3, y = 1/2),
            color = "red",
            mapping = aes(x = x, y = y),
