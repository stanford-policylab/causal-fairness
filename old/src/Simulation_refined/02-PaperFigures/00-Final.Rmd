...
df_test$ml_naive2 = df_test$A_raw
print_results(df_test, df_test$ml_naive, POP_SIZE_TEST_ADMIT, 'ML_NAIVE')
print_results(df_test, df_test$ml_naive2, POP_SIZE_TEST_ADMIT, 'ML_NAIVE2')
...
p <- ggplot() +
  ...
  scale_x_continuous("Class Diversity (% People of Color)", labels = scales::percent) + 
  ylab("Number of Individuals Passing Boards") +  
...
EO_FAIR = c('Counterfactual Equalized Odds', 22666.5,  0.45276453833690733, 22666.5)
PRINCIPAL_FAIR = c('Principal Fairness', 22322,  0.455, 22322)
...
p <- ggplot() +
  ...
  scale_x_continuous("Class Diversity (% People of Color)", labels = scales::percent_format(accuracy = 5L)) + 
  scale_y_continuous("Number of Individuals Passing Boards",labels = scales::comma) +
  coord_cartesian(xlim = c(0.0, 0.65),ylim = c(21750, 23500),expand = FALSE) + 

  geom_point(aes(x = as.double(ML_MAX_BOARDS_PASSAGE[[3]]), y = as.double(ML_MAX_BOARDS_PASSAGE[[4]])), color = peach, size = 3) +
  annotate("text",size=5, x = as.double(ML_MAX_BOARDS_PASSAGE[[3]])+0.05+0.075-0.01, y = as.double(ML_MAX_BOARDS_PASSAGE[[4]]) +50, label = ML_MAX_BOARDS_PASSAGE[[1]]) +

  geom_point(aes(x = as.double(ML_MAX_UTILITY[[3]]), y = as.double(ML_MAX_UTILITY[[4]])), color = blue, size = 3) +
  annotate("text",size=5, x = as.double(ML_MAX_UTILITY[[3]])-0.05-0.02+0.075, y = as.double(ML_MAX_UTILITY[[4]])-100 , label = ML_MAX_UTILITY[[1]]) +
  
  geom_point(aes(x = as.double(ML_PREDICT_PAR[[3]]), y = as.double(ML_PREDICT_PAR[[4]])), color = green, size = 3) +
  annotate("text",size=5, x = as.double(ML_PREDICT_PAR[[3]])+0.08+0.06, y = as.double(ML_PREDICT_PAR[[4]]) +25, label = ML_PREDICT_PAR[[1]]) +
  
  geom_point(aes(x = as.double(EO_FAIR[[3]]), y = as.double(EO_FAIR[[4]])), color = browngreen, size = 3) +
  annotate("text",size=5,hjust = 1,lineheight = 0.85, x = as.double(EO_FAIR[[3]])-0.03+0.02, y = as.double(EO_FAIR[[4]])-50-50 , label = EO_FAIR[[1]]) +

  geom_point(aes(x = as.double(PRINCIPAL_FAIR[[3]]), y = as.double(PRINCIPAL_FAIR[[4]])), color = darkgreen, size = 3) +
  annotate("text",size=5,hjust = 1,lineheight = 0.85, x = as.double(PRINCIPAL_FAIR[[3]])-0.03+0.02, y = as.double(PRINCIPAL_FAIR[[4]])-50-50 , label = PRINCIPAL_FAIR[[1]]) +
...
ggsave(plot=p, filename='./01_analogues.pdf', height=5, width=7)
p
...
p <- ggplot() +
  ...
  scale_x_continuous("Class Diversity (% People of Color)", labels = scales::percent_format(accuracy = 5L)) + 
  scale_y_continuous("Number of Individuals Passing Boards",labels = scales::comma) +
  coord_cartesian(xlim = c(0.0, 0.65),ylim = c(14500, 24000),expand = FALSE) + 
 # geom_vline(xintercept = as.double(ML_MAX_BOARDS_PASSAGE[[3]]), linetype = "dashed", colour = peach, alpha=transparency) +
#  geom_hline(yintercept = as.double(ML_MAX_BOARDS_PASSAGE[[4]]), linetype = "dashed", colour = peach, alpha=transparency) +
  geom_point(aes(x = as.double(ML_MAX_BOARDS_PASSAGE[[3]]), y = as.double(ML_MAX_BOARDS_PASSAGE[[4]])), color = peach, size = 3) +
  annotate("text",size=5, x = as.double(ML_MAX_BOARDS_PASSAGE[[3]])-0.065-0.035-0.02+0.125+0.125, y = as.double(ML_MAX_BOARDS_PASSAGE[[4]])- 200+250+250, label = ML_MAX_BOARDS_PASSAGE[[1]]) +

  #geom_vline(xintercept = as.double(ML_MAX_UTILITY[[3]]), linetype = "dashed", colour = blue, alpha=transparency) +
 # geom_hline(yintercept = as.double(ML_MAX_UTILITY[[4]]), linetype = "dashed", colour = blue, alpha=transparency) +
  geom_point(aes(x = as.double(ML_MAX_UTILITY[[3]]), y = as.double(ML_MAX_UTILITY[[4]])), color = blue, size = 3) +
  annotate("text",size=5, x = as.double(ML_MAX_UTILITY[[3]])-0.06+0.075-0.01, y = as.double(ML_MAX_UTILITY[[4]])-500, label = ML_MAX_UTILITY[[1]]) +
  
 # geom_vline(xintercept = as.double(HUMAN_RESULTS[[3]]), linetype = "dashed", colour = navy, alpha=transparency) +
 # geom_hline(yintercept = as.double(HUMAN_RESULTS[[4]]), linetype = "dashed", colour = navy, alpha=transparency) +
  geom_point(aes(x = as.double(HUMAN_RESULTS[[3]]), y = as.double(HUMAN_RESULTS[[4]])), color = navy, size = 3) +
  annotate("text", size=5,x = as.double(HUMAN_RESULTS[[3]])-0.09-0.015, y = as.double(HUMAN_RESULTS[[4]]) - 50 + 100, label = HUMAN_RESULTS[[1]]) +
  
 # geom_vline(xintercept = as.double(ML_NAIVE_RESULTS[[3]]), linetype = "dashed", colour = darkpurple, alpha=transparency) +
 # geom_hline(yintercept = as.double(ML_NAIVE_RESULTS[[4]]), linetype = "dashed", colour = darkpurple, alpha=transparency) +
  geom_point(aes(x = as.double(ML_NAIVE_RESULTS[[3]]), y = as.double(ML_NAIVE_RESULTS[[4]])), color = darkpurple, size = 3) +
  annotate("text",size=5, x = as.double(ML_NAIVE_RESULTS[[3]])-0.06, y = as.double(ML_NAIVE_RESULTS[[4]]) - 50, label = ML_NAIVE_RESULTS[[1]]) +
 ...
 # geom_hline(yintercept = as.double(PC_FAIR[[4]]), linetype = "dashed", colour = purple, alpha=transparency) +
  ...
  annotate("text",size=5,lineheight = 0.85,hjust = 1, x = as.double(PC_FAIR[[3]])-0.02, y = as.double(PC_FAIR[[4]]) - 50 + 100, label = PC_FAIR[[1]]) +
 ...
 # geom_hline(yintercept = as.double(RANDOM[[4]]), linetype = "dashed", colour = darkgreen, alpha=transparency) +
  geom_point(aes(x = as.double(PRINCIPAL_FAIR[[3]]), y = as.double(PRINCIPAL_FAIR[[4]])), color = darkgreen, size = 3) +
  annotate("text",size=5, x = as.double(PRINCIPAL_FAIR[[3]])-0.11-0.015+0.04, y = as.double(PRINCIPAL_FAIR[[4]]) - 50 + 100 - 500-100, label = PRINCIPAL_FAIR[[1]]) +
 ...
 # geom_hline(yintercept = as.double(ML_PREDICT_PAR[[4]]), linetype = "dashed", colour = green, alpha=transparency) +
  geom_point(aes(x = as.double(ML_PREDICT_PAR[[3]]), y = as.double(ML_PREDICT_PAR[[4]])), color = green, size = 3) +
  annotate("text",size=5, x = as.double(ML_PREDICT_PAR[[3]])+0.13+0.02, y = as.double(ML_PREDICT_PAR[[4]]) + 50, label = ML_PREDICT_PAR[[1]]) +
 ...
 # geom_hline(yintercept = as.double(EO_FAIR[[4]]), linetype = "dashed", colour = browngreen, alpha=transparency) +
  geom_point(aes(x = as.double(EO_FAIR[[3]]), y = as.double(EO_FAIR[[4]])), color = browngreen, size = 3) +
  annotate("text",size=5,lineheight = 0.85,hjust = 1, x = as.double(EO_FAIR[[3]])-0.01, y = as.double(EO_FAIR[[4]]) - 750+200+80, label = EO_FAIR[[1]]) +
  ...
  scale_color_manual(
      values = c(
           ...
           'ML Outcomes (Given T, R)'="violet" )) + 
  theme(legend.title = element_blank(),legend.position="none", axis.text=element_text(size=15), axis.title=element_text(size=18), panel.grid = element_line(color = rgb(235, 235, 235, 100, maxColorValue = 255)))
ggsave(plot=p, filename='./02_dags.pdf', height=5, width=7)
p
