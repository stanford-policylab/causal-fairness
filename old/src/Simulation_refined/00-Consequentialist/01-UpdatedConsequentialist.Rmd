...
df_test$ml_outcomes_div = df_test$ml_outcomes + 0.25*df_test$R
...
df_test$ml_eo_fair = FRAC_BLACK*predict(ml_outcomes,df_test_black) + (1-FRAC_BLACK)*predict(ml_outcomes,df_test_white)

print_results(df_test, df_test$ml_eo_fair, POP_SIZE_TEST_ADMIT, 'ML_EO_Fair')

ML_EO_FAIR_RESULTS = return_results(df_test, df_test$ml_eo_fair, POP_SIZE_TEST_ADMIT, 'Blinding / ML EO Fair')
...
p <- ggplot() +
  ...
  annotate("text", x = as.double(ML_MAX_BOARDS_PASSAGE[[3]])+0.05, y = as.double(ML_MAX_BOARDS_PASSAGE[[4]]) + 2, label = ML_MAX_BOARDS_PASSAGE[[1]]) +
  ...
  annotate("text", x = as.double(ML_MAX_UTILITY[[3]])+0.03, y = as.double(ML_MAX_UTILITY[[4]]) + 2, label = ML_MAX_UTILITY[[1]]) +
    ...
    geom_vline(xintercept = as.double(ML_EO_FAIR_RESULTS[[3]]), linetype = "dashed", colour = purple, alpha=transparency) +
  geom_hline(yintercept = as.double(ML_EO_FAIR_RESULTS[[4]]), linetype = "dashed", colour = purple, alpha=transparency) +
  geom_point(aes(x = as.double(ML_EO_FAIR_RESULTS[[3]]), y = as.double(ML_EO_FAIR_RESULTS[[4]])), color = purple, size = 3) +
  annotate("text", x = as.double(ML_EO_FAIR_RESULTS[[3]])-0.05, y = as.double(ML_EO_FAIR_RESULTS[[4]]) + 2, label = ML_EO_FAIR_RESULTS[[1]]) +
