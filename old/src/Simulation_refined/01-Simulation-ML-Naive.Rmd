...
## Enforce principal fairness

```{r ml_principal_fair}
df_fair <- df_test[,c('R','A','T')]




df_fair_ <- df_fair %>% count(R, A, T, sort = TRUE)
...
ml_naive_pf <- glm(A ~ T + I_noise, data = df_train, family = "binomial", control = list(maxit = 1000))

df_test$ml_naive_pf = plogis(predict(ml_naive_pf,df_test))
...
p <- ggplot() +
  ...
  annotate("text", x = as.double(ML_EO_FAIR_RESULTS[[3]])+0.03, y = as.double(ML_EO_FAIR_RESULTS[[4]]) + 2, label = ML_EO_FAIR_RESULTS[[1]]) +
  
  
  geom_vline(xintercept = as.double(HUMAN_RESULTS[[3]]), linetype = "dashed", colour = browngreen, alpha=transparency) +
  geom_hline(yintercept = as.double(HUMAN_RESULTS[[4]]), linetype = "dashed", colour = browngreen, alpha=transparency) +
  geom_point(aes(x = as.double(HUMAN_RESULTS[[3]]), y = as.double(HUMAN_RESULTS[[4]])), color = browngreen, size = 3) +
  annotate("text", x = as.double(HUMAN_RESULTS[[3]])+0.03, y = as.double(HUMAN_RESULTS[[4]]) + 2, label = HUMAN_RESULTS[[1]]) +
  ...
  annotate("text", x = as.double(ML_NAIVE_RESULTS[[3]])+0.03, y = as.double(ML_NAIVE_RESULTS[[4]]) + 2, label = ML_NAIVE_RESULTS[[1]]) +
